...
import { z } from "zod";
//#region src/host/auto-sync.ts
function pushAutoSyncUpdates(store, spawner, logger, sessionId, digest) {
	const sourceCardIds = /* @__PURE__ */ new Set();
	for (const [cardId, card] of store.cards.entries()) if (card.sessionId === sessionId) sourceCardIds.add(cardId);
	if (sourceCardIds.size === 0) return;
	const pushedTo = /* @__PURE__ */ new Set();
...
function formatUpdate(fromTitle, digest) {
