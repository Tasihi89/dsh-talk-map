window.__ModuleLoader__.load({ id: "dsh-talk-map", factory: (require) => {
		...
		const talkMapApi = {
			...
			injectContext(sessionId, parents) {
				return postJson("/talk-map/inject", {
					sessionId,
					parents
				});
			},
		...
		*/
		function t(key, params) {
			const template = (activeLocale() === "zh" ? zh : en)[key] ?? en[key] ?? key;
		...
		*/
		...
		function setState(next) {
			...
			setLocalePref(state$1.global?.locale ?? "auto");
		...
		function restoreSnapshot(snapshot) {
			const previousCards = state$1.cards;
			const previousEdges = state$1.edges;
			const currentCamera = state$1.global?.cameraByBoard;
			const global = snapshot.global === null ? state$1.global : {
				...snapshot.global,
				cameraByBoard: currentCamera ?? snapshot.global.cameraByBoard
			};
			setState({
			...
			const goneCards = Object.keys(previousCards).filter((id) => snapshot.cards[id] === void 0);
			const goneEdges = Object.keys(previousEdges).filter((id) => snapshot.edges[id] === void 0);
			(async () => {
		...
		*/
		...
		const canvas = {
			...
			ensureLoaded() {
				if (state$1.phase === "loading" || state$1.phase === "ready") return;
				setState({
					phase: "loading",
					error: void 0
				});
			...
			* positions or a global patch — are still in flight. */
			...
			setCardsColor(ids, colorTag) {
				...
				for (const id of ids) {
					...
					const next = { ...card };
					if (colorTag === void 0) delete next.colorTag;
			...
			setCardWorkspaceOverride(id, wsOverride) {
				...
				talkMapApi.upsertCards({ [id]: next }).catch((error) => {
			...
			savedCamera(boardId) {
				return state$1.global?.cameraByBoard[boardId];
			},
			/** First card found for a session (M1: one board, at most one card each). */
			cardIdForSession(sessionId) {
				for (const [id, card] of Object.entries(state$1.cards)) if (card.sessionId === sessionId) return id;
			}
		};
		function newCardId() {
		...
		function hotkeyMatches(event, value) {
			...
			return event.altKey === hotkey.alt && event.ctrlKey === hotkey.ctrl && event.metaKey === hotkey.meta && event.shiftKey === hotkey.shift && event.code === hotkey.code;
		}
		...
		function hotkeyLabel(value) {
			const hotkey = parseHotkey(value);
			const key = hotkey.code.startsWith("Key") ? hotkey.code.slice(3) : hotkey.code.startsWith("Digit") ? hotkey.code.slice(5) : hotkey.code;
		...
		*/
		...
		//#endregion
		//#region node_modules/.pnpm/@xyflow+react@12.11.3_@types+react-dom@18.3.7_@types+react@18.3.31__@types+react@18.3.3_71dfa8247134ab33b4cd4f58b02b90e3/node_modules/@xyflow/react/dist/esm/index.js
		...
		*/
		...
		function colorOf(tagId) {
			if (tagId === void 0) return void 0;
		...
		const css = ".UpoQZW_toggleButton{width:32px;height:32px;color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border:none;border-radius:8px;justify-content:center;align-items:center;gap:6px;padding:0;display:flex}.UpoQZW_toggleButton:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.UpoQZW_toggleButtonActive{background:var(--dsw-alias-interactive-bg-active);color:var(--dsw-alias-brand-primary)}.UpoQZW_toggleRow{width:100%;color:var(--dsw-alias-label-secondary);cursor:pointer;text-align:left;background:0 0;border:none;border-radius:8px;align-items:center;gap:8px;padding:6px 10px;font-size:13px;display:flex}.UpoQZW_toggleRow:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.UpoQZW_toggleLabel{white-space:nowrap}.UpoQZW_toggleIcon{flex:none;width:18px;height:18px}.UpoQZW_overlay{background:var(--dsw-alias-bg-base);color:var(--dsw-alias-label-primary);pointer-events:auto;flex-direction:column;display:flex;position:absolute;top:0;bottom:0;left:0;right:0}.UpoQZW_header{border-bottom:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1);flex:none;align-items:center;gap:10px;padding:8px 14px;display:flex}.UpoQZW_headerTitle{color:var(--dsw-alias-label-primary);font-size:14px;font-weight:600}.UpoQZW_headerBadge{color:var(--dsw-alias-label-tertiary);font-size:12px}.UpoQZW_headerSpace{flex:1}.UpoQZW_hotkeyButton{border:1px solid var(--dsw-alias-border-l2);color:var(--dsw-alias-label-secondary);cursor:pointer;white-space:nowrap;background:0 0;border-radius:999px;flex:none;padding:3px 10px;font-size:12px}.UpoQZW_hotkeyButton:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.UpoQZW_langButton{border:1px solid var(--dsw-alias-border-l2);min-width:58px;color:var(--dsw-alias-label-secondary);cursor:pointer;white-space:nowrap;background:0 0;border-radius:999px;flex:none;padding:3px 10px;font-size:12px}.UpoQZW_langButton:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.UpoQZW_zoomResetLabel{letter-spacing:-.5px;font-size:9px;font-weight:700}.UpoQZW_closeButton{width:28px;height:28px;color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border:none;border-radius:6px;justify-content:center;align-items:center;padding:0;display:flex}.UpoQZW_closeButton:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.UpoQZW_canvas{flex:1;min-height:0;position:relative}.UpoQZW_canvas .react-flow__nodesselection{pointer-events:none!important;display:none!important}.UpoQZW_canvas .react-flow__nodesselection-rect{pointer-events:none!important;display:none!important}.UpoQZW_card{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-1);width:224px;min-height:104px;color:var(--dsw-alias-label-primary);box-shadow:0 1px 3px var(--dsw-alias-bg-mask-1);cursor:pointer;border-radius:10px;flex-direction:column;gap:6px;padding:10px 12px;font-size:13px;transition:box-shadow .12s;display:flex}.UpoQZW_card:hover{box-shadow:0 4px 12px var(--dsw-alias-bg-mask-2)}.UpoQZW_cardSelected{border-color:var(--dsw-alias-brand-primary)}.UpoQZW_cardGhost{opacity:.55;cursor:default;border-style:dashed}.UpoQZW_cardTop{align-items:center;gap:6px;min-width:0;display:flex}.UpoQZW_cardTitle{-webkit-line-clamp:2;word-break:break-word;-webkit-box-orient:vertical;font-size:13px;font-weight:600;line-height:1.3;display:-webkit-box;overflow:hidden}.UpoQZW_runningDot{background:var(--dsw-alias-state-success-primary);border-radius:50%;flex:none;width:8px;height:8px;animation:1.6s ease-in-out infinite UpoQZW_talkmap-pulse}@keyframes UpoQZW_talkmap-pulse{50%{opacity:.35}}.UpoQZW_cardNext{color:var(--dsw-alias-label-secondary);align-items:baseline;gap:6px;min-width:0;font-size:12px;display:flex}.UpoQZW_cardNextLabel{color:var(--dsw-alias-brand-primary);flex:none;font-size:11px;font-weight:600}.UpoQZW_cardNextText{-webkit-line-clamp:2;-webkit-box-orient:vertical;display:-webkit-box;overflow:hidden}.UpoQZW_cardStale{color:var(--dsw-alias-state-warn-primary);flex:none;font-size:12px}.UpoQZW_cardFooter{color:var(--dsw-alias-label-tertiary);justify-content:space-between;align-items:center;gap:8px;margin-top:auto;font-size:11px;display:flex}.UpoQZW_cardTime{flex:none}.UpoQZW_cardBadgeRunning{color:var(--dsw-alias-state-success-primary);background:var(--dsw-alias-state-success-tertiary);border-radius:999px;align-items:center;gap:5px;padding:1px 7px;font-size:10px;display:inline-flex}.UpoQZW_cardSummary{color:var(--dsw-alias-label-secondary);-webkit-line-clamp:2;word-break:break-word;-webkit-box-orient:vertical;font-size:12px;line-height:1.45;display:-webkit-box;overflow:hidden}.UpoQZW_cardBadgeWaiting{color:var(--dsw-alias-state-warn-label);background:var(--dsw-alias-state-warn-tertiary);border-radius:999px;padding:1px 7px;font-size:10px}.UpoQZW_cardBadgeDone{color:var(--dsw-alias-state-success-primary);background:var(--dsw-alias-state-success-tertiary);border-radius:999px;padding:1px 7px;font-size:10px}.UpoQZW_cardRemove{border:1px solid var(--dsw-alias-border-l2);color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border-radius:6px;align-self:flex-start;padding:3px 8px;font-size:12px}.UpoQZW_cardRemove:hover{background:var(--dsw-alias-interactive-bg-hover-danger);color:var(--dsw-alias-state-error-primary)}.UpoQZW_cardHandle{background:var(--dsw-alias-border-l3);border:2px solid var(--dsw-alias-bg-layer-1);opacity:0;width:10px;height:10px}.UpoQZW_card:hover .UpoQZW_cardHandle,.UpoQZW_canvasConnecting .UpoQZW_cardHandle{opacity:1}.UpoQZW_cardHandle:hover{background:var(--dsw-alias-brand-primary)}.UpoQZW_cardRefresh{width:22px;height:22px;color:var(--dsw-alias-label-tertiary);cursor:pointer;opacity:0;background:0 0;border:none;border-radius:6px;flex:none;margin-left:auto;padding:0;font-size:13px;line-height:1;transition:opacity .12s}.UpoQZW_card:hover .UpoQZW_cardRefresh{opacity:1}.UpoQZW_cardRefresh:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.UpoQZW_cardRefreshBusy{opacity:1;animation:1s linear infinite UpoQZW_talkmap-spin}@keyframes UpoQZW_talkmap-spin{to{transform:rotate(360deg)}}.UpoQZW_spawnPanel{z-index:6;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-1);width:420px;max-width:calc(100% - 48px);box-shadow:0 8px 30px var(--dsw-alias-bg-mask-2);border-radius:12px;flex-direction:column;gap:8px;padding:16px;display:flex;position:absolute;top:24px;right:24px}.UpoQZW_spawnHeading{color:var(--dsw-alias-label-primary);font-size:14px;font-weight:600}.UpoQZW_spawnFrom{color:var(--dsw-alias-label-secondary);font-size:12px}.UpoQZW_spawnModes{gap:6px;display:flex}.UpoQZW_spawnModeBtn{border:1px solid var(--dsw-alias-border-l2);color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border-radius:8px;flex:1;padding:5px 0;font-size:12px}.UpoQZW_spawnModeBtn:hover{background:var(--dsw-alias-interactive-bg-hover)}.UpoQZW_spawnModeActive{border-color:var(--dsw-alias-brand-primary);color:var(--dsw-alias-brand-primary);background:var(--dsw-alias-interactive-bg-active)}.UpoQZW_spawnHint{color:var(--dsw-alias-label-tertiary);font-size:12px}.UpoQZW_spawnTextarea{resize:vertical;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-base);width:100%;min-height:140px;color:var(--dsw-alias-label-primary);border-radius:8px;padding:10px;font-family:inherit;font-size:13px;line-height:1.5}.UpoQZW_spawnError{color:var(--dsw-alias-state-error-primary);word-break:break-all;font-size:12px}.UpoQZW_spawnActions{justify-content:flex-end;gap:8px;display:flex}.UpoQZW_spawnBtnPrimary{background:var(--dsw-alias-button-primary-fill);color:var(--dsw-alias-label-primary-foreground);cursor:pointer;border:none;border-radius:8px;padding:6px 16px;font-size:13px}.UpoQZW_spawnBtnPrimary:hover{background:var(--dsw-alias-button-primary-hover)}.UpoQZW_spawnBtnPrimary:disabled{background:var(--dsw-alias-button-primary-dimmed);cursor:default}.UpoQZW_spawnBtnGhost{border:1px solid var(--dsw-alias-border-l2);color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border-radius:8px;padding:6px 16px;font-size:13px}.UpoQZW_spawnBtnGhost:hover{background:var(--dsw-alias-interactive-bg-hover)}.UpoQZW_wsFrame{border:1.5px dashed var(--dsw-alias-border-l2);background:0 0;border-radius:16px;position:relative}.UpoQZW_wsFrameSelected{border-style:solid;border-color:var(--dsw-alias-brand-primary)}.UpoQZW_wsFrameLabel{border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1);color:var(--dsw-alias-label-secondary);white-space:nowrap;pointer-events:auto;cursor:grab;border-radius:999px;align-items:center;gap:6px;padding:2px 10px;font-size:12px;font-weight:600;display:flex;position:absolute;top:-12px;left:14px}.UpoQZW_wsFrameLabel:active{cursor:grabbing}.UpoQZW_wsFrameEdge{pointer-events:auto;cursor:grab;position:absolute}.UpoQZW_wsFrameEdge:active{cursor:grabbing}.UpoQZW_wsFrameResize{pointer-events:auto;cursor:nwse-resize;border-right:3px solid var(--dsw-alias-border-l3);border-bottom:3px solid var(--dsw-alias-border-l3);opacity:.7;border-bottom-right-radius:14px;width:16px;height:16px;position:absolute;bottom:-2px;right:-2px}.UpoQZW_wsFrameResize:hover{border-color:var(--dsw-alias-brand-primary);opacity:1}.UpoQZW_wsFrameCount{color:var(--dsw-alias-label-tertiary);font-weight:400}.UpoQZW_draftCard{box-sizing:border-box;border:1.5px solid var(--dsw-alias-brand-primary);background:var(--dsw-alias-bg-layer-1);width:360px;box-shadow:0 8px 30px var(--dsw-alias-bg-mask-2);border-radius:12px;flex-direction:column;gap:8px;padding:14px;font-size:13px;display:flex}.UpoQZW_draftHeading{color:var(--dsw-alias-label-primary);font-size:13px;font-weight:600}.UpoQZW_draftRow{align-items:center;gap:8px;display:flex}.UpoQZW_draftLabel{width:52px;color:var(--dsw-alias-label-tertiary);flex:none;font-size:12px}.UpoQZW_draftSelect{box-sizing:border-box;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-base);min-width:0;color:var(--dsw-alias-label-primary);border-radius:7px;flex:1;padding:5px 8px;font-size:12px}.UpoQZW_draftGrow{flex-direction:column;flex:1;gap:3px;min-width:0;display:flex}.UpoQZW_draftInput{box-sizing:border-box;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-base);width:100%;color:var(--dsw-alias-label-primary);border-radius:7px;padding:5px 8px;font-family:inherit;font-size:12px}.UpoQZW_draftPathPreview{color:var(--dsw-alias-label-tertiary);word-break:break-all;font-size:11px}.UpoQZW_draftTextarea{box-sizing:border-box;resize:vertical;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-base);width:100%;min-height:88px;color:var(--dsw-alias-label-primary);border-radius:8px;padding:9px;font-family:inherit;font-size:13px;line-height:1.5}.UpoQZW_colorToolbar{z-index:7;border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1);box-shadow:0 2px 10px var(--dsw-alias-bg-mask-1);border-radius:10px;align-items:center;gap:6px;padding:6px 10px;display:flex;position:absolute;top:12px;left:12px}.UpoQZW_colorToolbarLabel{color:var(--dsw-alias-label-tertiary);margin-right:2px;font-size:12px}.UpoQZW_colorSwatch{border:1px solid var(--dsw-alias-border-l2);cursor:pointer;width:18px;height:18px;color:var(--dsw-alias-label-secondary);border-radius:50%;justify-content:center;align-items:center;padding:0;font-size:11px;line-height:1;display:flex}.UpoQZW_colorSwatch:hover{transform:scale(1.15)}.UpoQZW_colorSwatchClear{background:0 0}.UpoQZW_menu{z-index:9;border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1);min-width:200px;max-width:280px;max-height:380px;box-shadow:0 8px 30px var(--dsw-alias-bg-mask-2);border-radius:10px;flex-direction:column;padding:6px;display:flex;position:absolute;overflow-y:auto}.UpoQZW_menuTitle{color:var(--dsw-alias-label-tertiary);flex:none;padding:4px 10px 6px;font-size:11px}.UpoQZW_menuSearch{box-sizing:border-box;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-base);width:calc(100% - 8px);color:var(--dsw-alias-label-primary);border-radius:7px;flex:none;margin:2px 4px 6px;padding:5px 8px;font-family:inherit;font-size:12px}.UpoQZW_menuHint{color:var(--dsw-alias-label-dimmed);font-size:11px}.UpoQZW_menuPreview{color:var(--dsw-alias-label-tertiary);word-break:break-all;flex:none;padding:0 10px 4px;font-size:11px}.UpoQZW_menuError{color:var(--dsw-alias-state-error-primary);word-break:break-all;flex:none;padding:0 10px 4px;font-size:11px}.UpoQZW_menuEmpty{color:var(--dsw-alias-label-tertiary);padding:8px 10px;font-size:12px}.UpoQZW_menuItem{text-align:left;width:100%;color:var(--dsw-alias-label-primary);cursor:pointer;white-space:nowrap;text-overflow:ellipsis;background:0 0;border:none;border-radius:7px;flex:none;padding:6px 10px;font-size:13px;display:block;overflow:hidden}.UpoQZW_menuItem:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover)}.UpoQZW_menuItem:disabled{color:var(--dsw-alias-label-dimmed);cursor:default}.UpoQZW_wsFrameHidden{color:var(--dsw-alias-state-warn-primary);font-weight:400}.UpoQZW_emptyHint{color:var(--dsw-alias-label-tertiary);pointer-events:none;z-index:4;font-size:13px;position:absolute;top:50%;left:50%;transform:translate(-50%,-50%)}.UpoQZW_toast{z-index:8;border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1);max-width:70%;box-shadow:0 4px 16px var(--dsw-alias-bg-mask-2);color:var(--dsw-alias-label-primary);pointer-events:none;white-space:nowrap;text-overflow:ellipsis;border-radius:999px;padding:7px 16px;font-size:12px;position:absolute;bottom:28px;left:50%;overflow:hidden;transform:translate(-50%)}";
		...
		var talk_map_module_css_default = {
			"draftRow": "UpoQZW_draftRow",
			"draftPathPreview": "UpoQZW_draftPathPreview",
			"menu": "UpoQZW_menu",
			"menuError": "UpoQZW_menuError",
			"langButton": "UpoQZW_langButton",
			"toggleRow": "UpoQZW_toggleRow",
			"cardFooter": "UpoQZW_cardFooter",
			"canvasConnecting": "UpoQZW_canvasConnecting",
			"toggleButton": "UpoQZW_toggleButton",
			"cardGhost": "UpoQZW_cardGhost",
			"draftGrow": "UpoQZW_draftGrow",
			"cardSummary": "UpoQZW_cardSummary",
			"draftHeading": "UpoQZW_draftHeading",
			"cardTime": "UpoQZW_cardTime",
			"header": "UpoQZW_header",
			"spawnFrom": "UpoQZW_spawnFrom",
			"menuItem": "UpoQZW_menuItem",
			"cardNextText": "UpoQZW_cardNextText",
			"spawnActions": "UpoQZW_spawnActions",
			"spawnModeActive": "UpoQZW_spawnModeActive",
			"menuHint": "UpoQZW_menuHint",
			"cardSelected": "UpoQZW_cardSelected",
			"spawnPanel": "UpoQZW_spawnPanel",
			"toggleIcon": "UpoQZW_toggleIcon",
			"overlay": "UpoQZW_overlay",
			"wsFrameResize": "UpoQZW_wsFrameResize",
			"toast": "UpoQZW_toast",
			"toggleButtonActive": "UpoQZW_toggleButtonActive",
			"closeButton": "UpoQZW_closeButton",
			"talkmap-pulse": "UpoQZW_talkmap-pulse",
			"cardNextLabel": "UpoQZW_cardNextLabel",
			"cardStale": "UpoQZW_cardStale",
			"cardRemove": "UpoQZW_cardRemove",
			"spawnHint": "UpoQZW_spawnHint",
			"spawnBtnGhost": "UpoQZW_spawnBtnGhost",
			"spawnTextarea": "UpoQZW_spawnTextarea",
			"spawnModes": "UpoQZW_spawnModes",
			"draftInput": "UpoQZW_draftInput",
			"emptyHint": "UpoQZW_emptyHint",
			"zoomResetLabel": "UpoQZW_zoomResetLabel",
			"spawnModeBtn": "UpoQZW_spawnModeBtn",
			"wsFrameLabel": "UpoQZW_wsFrameLabel",
			"cardTitle": "UpoQZW_cardTitle",
			"cardNext": "UpoQZW_cardNext",
			"colorToolbar": "UpoQZW_colorToolbar",
			"wsFrameSelected": "UpoQZW_wsFrameSelected",
			"draftLabel": "UpoQZW_draftLabel",
			"runningDot": "UpoQZW_runningDot",
			"talkmap-spin": "UpoQZW_talkmap-spin",
			"menuTitle": "UpoQZW_menuTitle",
			"headerTitle": "UpoQZW_headerTitle",
			"draftSelect": "UpoQZW_draftSelect",
			"draftTextarea": "UpoQZW_draftTextarea",
			"wsFrame": "UpoQZW_wsFrame",
			"wsFrameEdge": "UpoQZW_wsFrameEdge",
			"cardBadgeWaiting": "UpoQZW_cardBadgeWaiting",
			"cardBadgeDone": "UpoQZW_cardBadgeDone",
			"menuEmpty": "UpoQZW_menuEmpty",
			"cardBadgeRunning": "UpoQZW_cardBadgeRunning",
			"spawnError": "UpoQZW_spawnError",
			"draftCard": "UpoQZW_draftCard",
			"card": "UpoQZW_card",
			"cardHandle": "UpoQZW_cardHandle",
			"cardRefresh": "UpoQZW_cardRefresh",
			"colorSwatchClear": "UpoQZW_colorSwatchClear",
			"headerSpace": "UpoQZW_headerSpace",
			"menuSearch": "UpoQZW_menuSearch",
			"headerBadge": "UpoQZW_headerBadge",
			"wsFrameHidden": "UpoQZW_wsFrameHidden",
			"spawnHeading": "UpoQZW_spawnHeading",
			"canvas": "UpoQZW_canvas",
			"wsFrameCount": "UpoQZW_wsFrameCount",
			"cardTop": "UpoQZW_cardTop",
			"colorToolbarLabel": "UpoQZW_colorToolbarLabel",
			"menuPreview": "UpoQZW_menuPreview",
			"colorSwatch": "UpoQZW_colorSwatch",
			"cardRefreshBusy": "UpoQZW_cardRefreshBusy",
			"spawnBtnPrimary": "UpoQZW_spawnBtnPrimary",
			"hotkeyButton": "UpoQZW_hotkeyButton",
			"toggleLabel": "UpoQZW_toggleLabel"
		};
		//#endregion
		...
		*/
		function ContextMenu(props) {
			...
			const submitPrompt = async () => {
				...
				if (prompt === void 0 || busy || query.trim() === "") return;
				setBusy(true);
				setError(void 0);
				try {
			...
			(0, react.useEffect)(() => {
				if (menu.searchable === true || menu.prompt !== void 0) inputRef.current?.focus();
			}, [menu.searchable, menu.prompt]);
			const normalized = query.trim().toLowerCase();
			const visible = normalized === "" ? menu.items : menu.items.filter((item) => item.label.toLowerCase().includes(normalized));
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				className: talk_map_module_css_default["menu"],
				style: {
					left: menu.left,
					top: menu.top
				},
				...
				children: [
					menu.title !== void 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						className: talk_map_module_css_default["menuTitle"],
						children: menu.title
					}) : null,
					menu.prompt !== void 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
							ref: inputRef,
							className: talk_map_module_css_default["menuSearch"],
						...
						menu.prompt.preview !== void 0 && query.trim() !== "" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							className: talk_map_module_css_default["menuPreview"],
							children: menu.prompt.preview(query.trim())
						}) : null,
						error !== void 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							className: talk_map_module_css_default["menuError"],
							children: error
						}) : null,
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
							type: "button",
							className: talk_map_module_css_default["menuItem"],
							disabled: busy || query.trim() === "",
							onClick: () => {
								submitPrompt();
							},
							children: busy ? t("menu.working") : menu.prompt.submitLabel
						})
					] }) : null,
					menu.searchable === true ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
						ref: inputRef,
						className: talk_map_module_css_default["menuSearch"],
					...
					menu.prompt !== void 0 ? null : visible.length === 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						className: talk_map_module_css_default["menuEmpty"],
						...
						children: [item.label, item.hint !== void 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
							className: talk_map_module_css_default["menuHint"],
							children: [" ", item.hint]
						}) : null]
					}, item.key))
				]
		...
		*/
		...
		function unwrap$1(response, what) {
		...
		function newWorkspaceParent(options) {
		...
		function DraftCardNode(props) {
			const { data } = props;
			const [workspaceId, setWorkspaceId] = (0, react.useState)(data.defaultWorkspaceId ?? NEW_WORKSPACE);
			const [newWsName, setNewWsName] = (0, react.useState)("");
			...
			const [defaultModelLabel, setDefaultModelLabel] = (0, react.useState)("");
			const [defaultPresetLabel, setDefaultPresetLabel] = (0, react.useState)("");
			const [busy, setBusy] = (0, react.useState)(false);
			const [error, setError] = (0, react.useState)(void 0);
			...
			(0, react.useEffect)(() => {
				...
				services.connection.api.agentPresets.list({}).then((response) => {
					if (response.result.ok) {
						...
						setPresets(list);
						if (list.some((preset) => preset.id === "standard")) setPresetId((previous) => previous === "" ? "standard" : previous);
					}
				}).catch(() => void 0);
			...
			const newWsPath = `${newWorkspaceParent(data.workspaceOptions)}/${newWsName.trim()}`;
			const sendDisabled = busy || text.trim() === "" || creatingWorkspace && newWsName.trim() === "";
			const send = async () => {
				const services = getServices();
				if (services === void 0 || sendDisabled) return;
				setBusy(true);
				setError(void 0);
				try {
					...
					if (creatingWorkspace) {
						...
						targetWorkspaceId = unwrap$1(await services.connection.api.workspace.create({ path: newWsPath }), "workspace.create").workspace.workspaceId;
					}
					const sessionId = unwrap$1(await services.connection.api.sessions.create({
						workspaceId: targetWorkspaceId,
						...presetId !== "" ? { agentPreset: presetId } : {}
					}), "session.create").sessionId;
					const cardX = snap$1(props.positionAbsoluteX);
					const cardY = snap$1(props.positionAbsoluteY);
					...
					if (canvas.get().global?.wsFrames?.[targetWorkspaceId] === void 0) {
						const PAD = 32;
					...
					if (modelKey !== "") {
						...
						unwrap$1(await services.connection.api.sessions.selectModel({
							sessionId,
							provider,
							model
						}), "session.selectModel");
					}
					unwrap$1(await services.connection.api.sessions.prompt({
						sessionId,
						mode: "queue",
						content: [{
							type: "text",
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				onPointerDownCapture: (event) => {
					if (event.target.closest("select, textarea, input, button") !== null) event.stopPropagation();
				},
				...
				children: [
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("label", {
							...
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("option", {
								value: NEW_WORKSPACE,
								children: t("draft.newWorkspace")
							}), data.workspaceOptions.map((option) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("option", {
								value: option.id,
								children: option.title
							}, option.id))]
						})]
					}),
					creatingWorkspace ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: talk_map_module_css_default["draftRow"],
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("label", {
							className: talk_map_module_css_default["draftLabel"],
							children: t("draft.wsName")
						}), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							className: talk_map_module_css_default["draftGrow"],
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
								className: `${talk_map_module_css_default["draftInput"]} nodrag`,
								value: newWsName,
								placeholder: t("draft.wsNamePlaceholder"),
								onChange: (event) => {
									setNewWsName(event.target.value);
								}
							}), newWsName.trim() !== "" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
								className: talk_map_module_css_default["draftPathPreview"],
								children: newWsPath
							}) : null]
						})]
					}) : null,
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("label", {
							...
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("option", {
								value: "",
								children: defaultModelLabel !== "" ? `${defaultModelLabel}${t("draft.defaultSuffix")}` : t("draft.default")
					...
					presets.length > 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("label", {
							...
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("option", {
								value: "",
								children: defaultPresetLabel !== "" ? `${defaultPresetLabel}${t("draft.defaultSuffix")}` : t("draft.default")
		...
		function SessionCardNode(props) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				style: color !== void 0 ? {
					...
					backgroundImage: `linear-gradient(${color.fill}, ${color.fill})`
				} : {},
				children: [
					...
					}) : /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [data.summary !== void 0 && data.summary !== "" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						className: talk_map_module_css_default["cardSummary"],
						children: data.summary
					}) : null, data.nextStep !== void 0 && data.nextStep !== "" ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: talk_map_module_css_default["cardFooter"],
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
							className: talk_map_module_css_default["cardTime"],
							children: relativeTime(data.updatedAt)
						}), data.waiting === true ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
							className: talk_map_module_css_default["cardBadgeWaiting"],
							children: t("card.waiting")
						}) : data.running ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
							className: talk_map_module_css_default["cardBadgeRunning"],
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { className: talk_map_module_css_default["runningDot"] }), t("card.running")]
						}) : data.done === true ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
							className: talk_map_module_css_default["cardBadgeDone"],
		...
		*/
		...
		function SpawnPreview(props) {
			...
			const [mode, setMode] = (0, react.useState)("digest");
			const [text, setText] = (0, react.useState)(() => buildInjectionText(pending.parent.title, digest));
			const [firstMessage, setFirstMessage] = (0, react.useState)("");
			const [busy, setBusy] = (0, react.useState)(false);
			const [error, setError] = (0, react.useState)(void 0);
			(0, react.useEffect)(() => {
			...
			const addCardAndEdge = (sessionId, kind, injectedText) => {
			...
			const confirm = async () => {
				...
				try {
					if (mode === "digest") {
						...
						const created = unwrap(await services.connection.api.sessions.create(pending.parentWorkspaceId !== void 0 ? { workspaceId: pending.parentWorkspaceId } : {}), "session.create");
			...
			const modes = [
				{
					id: "none",
					label: t("spawn.mode.none")
				},
				{
					id: "digest",
					label: t("spawn.mode.digest")
				},
				{
					id: "full",
					label: t("spawn.mode.full")
				}
			];
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						className: talk_map_module_css_default["spawnModes"],
						children: modes.map((entry) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
							type: "button",
							className: `${talk_map_module_css_default["spawnModeBtn"]}${mode === entry.id ? ` ${talk_map_module_css_default["spawnModeActive"]}` : ""}`,
							onClick: () => {
								setMode(entry.id);
							},
					...
					mode === "digest" ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
					...
					})] }) : mode === "none" ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						className: talk_map_module_css_default["spawnHint"],
						children: t("spawn.noneHint")
					}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("textarea", {
						className: `${talk_map_module_css_default["spawnTextarea"]} nodrag nowheel`,
						placeholder: t("draft.placeholder"),
						value: firstMessage,
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
							...
							disabled: busy || mode === "digest" && text.trim() === "" || mode === "none" && firstMessage.trim() === "",
		...
		function WsFrameNode(props) {
			...
			const color = colorOf(data.colorTag);
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				className: `${talk_map_module_css_default["wsFrame"]}${selected === true ? ` ${talk_map_module_css_default["wsFrameSelected"]}` : ""}`,
				style: {
					width: data.width,
					height: data.height,
					...color !== void 0 ? {
				...
				children: [
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						className: talk_map_module_css_default["wsFrameEdge"],
						style: {
							top: 0,
							left: 0,
							right: 0,
		...
		*/
		...
		function CanvasInner(props) {
			...
			const connectCancelledRef = (0, react.useRef)(false);
			...
			const framePosRef = (0, react.useRef)({});
			...
			const frameGeometry = (0, react.useMemo)(() => {
				...
				for (const [groupId, stored] of Object.entries(wsFrames)) {
					const members = (membersByGroup.get(groupId) ?? []).map((id) => visibleCards[id]).filter((card) => card !== void 0);
			...
			(0, react.useEffect)(() => {
				framePosRef.current = Object.fromEntries(Object.entries(frameGeometry).map(([groupId, rect]) => [`frame-${groupId}`, {
					x: rect.x,
					y: rect.y
				}]));
			}, [frameGeometry]);
			...
			const liveNodes = (0, react.useMemo)(() => {
				const wsColors = canvasState.global?.wsColors ?? {};
				const frameNodes = Object.entries(frameGeometry).map(([groupId, rect]) => {
					...
					return {
						...
						data: {
							...
							width: rect.width,
							height: rect.height,
							...wsColors[groupId] !== void 0 ? { colorTag: wsColors[groupId] } : {}
						},
						draggable: true,
						selectable: false,
						focusable: false,
				...
				const cardNodes = Object.entries(visibleCards).filter(([, card]) => card.boardId === INBOX_BOARD_ID).map(([cardId, card]) => {
					...
					const data = {
						...
						waiting: summary?.pendingInteraction !== void 0,
				...
				const draftNodes = draft === null ? [] : [{
					...
					data: {
						...
						...draft.groupId !== void 0 ? { groupId: draft.groupId } : {},
						onClose: () => {
							setDraft(null);
						}
					}
			...
			const onNodesChange = (changes) => {
				...
				for (const change of changes) if (change.type === "position" && change.position !== void 0) {
					if (change.id === "draft") setDraft((previous) => previous === null ? previous : {
						...
						const groupId = change.id.slice(6);
						if (previous !== void 0) {
							const dx = change.position.x - previous.x;
							const dy = change.position.y - previous.y;
							if (dx !== 0 || dy !== 0) for (const cardId of membersByGroup.get(groupId) ?? []) {
								const card = canvasState.cards[cardId];
								if (card !== void 0) canvas.moveCard(cardId, card.x + dx, card.y + dy);
							}
						}
						framePosRef.current[change.id] = {
							x: change.position.x,
							y: change.position.y
						};
						const rect = wsFrames[groupId];
						if (rect !== void 0) canvas.setWsFrameRect(groupId, {
							x: change.position.x,
							y: change.position.y,
							width: rect.width,
							height: rect.height
					...
					if (change.id.startsWith("frame-")) removeGroup(change.id.slice(6));
					else if (change.id !== "draft") canvas.removeCard(change.id);
					setSelectedIds((previous) => {
						const next = new Set(previous);
			...
			* a second draw over an already-connected pair is silently a no-op. */
			...
			const onConnectEnd = (event, connectionState) => {
				...
				setPendingSpawn({
					...
					...parentWorkspaceId !== void 0 ? { parentWorkspaceId } : {},
			...
			const groupAtPoint = (x, y) => {
				for (const [groupId, rect] of Object.entries(wsFrames)) if (x >= rect.x && x <= rect.x + rect.width && y >= rect.y && y <= rect.y + rect.height) return groupId;
			};
			const openDraftAt = (flowX, flowY) => {
				const x = snap(flowX - CARD_W / 2);
				const y = snap(flowY - 40);
				const groupId = groupAtPoint(flowX, flowY);
				const isWorkspace = groupId !== void 0 && wsTitles.has(groupId);
				setDraft({
					x,
					y,
			...
			const syncGroup = (groupId) => {
				...
				const fresh = placeableSessionIds(sessions).filter((id) => sessionWs.get(id) === groupId && !boardSessionIds.has(id));
				if (fresh.length === 0) return;
				const members = (membersByGroup.get(groupId) ?? []).map((id) => canvasState.cards[id]).filter((card) => card !== void 0);
			...
			const menuState = (0, react.useMemo)(() => {
				...
				if (menu.view === "new-ws") {
					...
					return {
						...
						prompt: {
							...
							preview: (value) => `${parentDir}/${value}`,
							onSubmit: async (value) => {
								const services = getServices();
								if (services === void 0 || parentDir === "") throw new Error("no workspace root available");
								...
								if (!response.result.ok) throw new Error(`workspace.create: ${response.result.error.message ?? response.result.error.code ?? ""}`);
				...
				if (menu.view === "import-ws") {
					...
					for (const item of workspaces.items) {
						const imported = wsFrames[item.workspaceId] !== void 0;
						items.push({
					...
					const candidates = placeableSessionIds(sessions).filter((id) => !boardSessionIds.has(id)).slice(0, 200);
					for (const sessionId of candidates) items.push({
						key: sessionId,
				...
				} else if (menu.kind === "card" && menu.targetId !== void 0) {
					...
					items.push({
						...
						onPick: () => {
							close();
							if (card !== void 0) openSession(card.sessionId);
						}
					});
					items.push({
						key: "digest",
						...
						onPick: () => {
							close();
							if (card !== void 0) talkMapApi.refreshDigest(card.sessionId).catch(() => void 0);
						}
					});
					items.push({
						key: "remove",
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)(index, {
						...
						onNodeClick: (event, node) => {
							if (node.type === "sessionCard") {
								const cardData = node.data;
								if (!cardData.ghost) openSession(cardData.sessionId);
								return;
							}
							...
							setSelectedIds((previous) => {
								...
								return /* @__PURE__ */ new Set([frameId]);
							});
						},
						...
						...savedCamera !== void 0 ? { defaultViewport: savedCamera } : { fitView: hasContent },
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(Background, {
							...
							children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(ControlButton, {
								title: t("map.zoom100"),
								onClick: () => {
									zoomTo(1, { duration: 200 });
								},
								children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
									className: talk_map_module_css_default["zoomResetLabel"],
									children: "1:1"
								})
							})
						})]
					...
					menuState !== null ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(ContextMenu, {
						menu: menuState,
						onClose: () => {
							setMenu(null);
						}
		...
		function ColorToolbar(props) {
			const selectedCardIds = [...props.selectedIds].filter((id) => props.visibleCards[id] !== void 0);
			const selectedFrameWs = [...props.selectedIds].filter((id) => id.startsWith("frame-")).map((id) => id.slice(6));
			const applyColor = (colorTag) => {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				className: talk_map_module_css_default["colorToolbar"],
				role: "toolbar",
				"aria-label": t("color.toolbar"),
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
						className: talk_map_module_css_default["colorToolbarLabel"],
						children: t("color.toolbar")
					}),
					COLOR_TAGS.map((tag) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
						type: "button",
						className: talk_map_module_css_default["colorSwatch"],
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
						type: "button",
						className: `${talk_map_module_css_default["colorSwatch"]} ${talk_map_module_css_default["colorSwatchClear"]}`,
						title: t("color.clear"),
		...
		*/
		function HotkeyButton() {
			const hotkey = (0, react.useSyncExternalStore)(canvas.subscribe, () => canvas.get().global?.hotkey ?? "alt+KeyF");
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
				type: "button",
				className: talk_map_module_css_default["hotkeyButton"],
				title: t("hotkey.title"),
				onClick: () => {
					setCapturing(true);
				},
				children: capturing ? t("hotkey.capturing") : hotkeyLabel(hotkey)
			});
		}
		/**
		...
		*/
		function LanguageButton() {
			const pref = (0, react.useSyncExternalStore)(canvas.subscribe, () => canvas.get().global?.locale ?? "auto");
			const index = LOCALE_CYCLE.indexOf(pref);
			const next = LOCALE_CYCLE[(index + 1) % LOCALE_CYCLE.length] ?? "auto";
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
				type: "button",
				className: talk_map_module_css_default["langButton"],
				title: t("lang.hint", {
					current: t(`lang.${pref}`),
					next: t(`lang.${next}`)
				}),
				onClick: () => {
					canvas.patchGlobalNow({ locale: next });
				},
				children: t(`lang.${pref}`)
			});
		}
		function CloseIcon() {
		...
		function OpenMapOverlay(props) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				className: talk_map_module_css_default["overlay"],
				lang: activeLocale(),
				role: "dialog",
				"aria-label": t("map.title"),
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					className: talk_map_module_css_default["header"],
					children: [
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
							className: talk_map_module_css_default["headerTitle"],
							children: t("map.title")
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
							className: talk_map_module_css_default["headerBadge"],
							children: [
								sessionCount,
								" ",
								t("map.sessions")
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { className: talk_map_module_css_default["headerSpace"] }),
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)(LanguageButton, {}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)(HotkeyButton, {}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
							type: "button",
							className: talk_map_module_css_default["closeButton"],
							title: t("map.close"),
		...
		*/
		function MapIcon() {
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("svg", {
				className: talk_map_module_css_default["toggleIcon"],
				viewBox: "0 0 24 24",
				fill: "none",
				stroke: "currentColor",
				strokeWidth: "1.8",
				...
				"aria-hidden": "true",
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("path", { d: "M9 4 3.5 6v14L9 18l6 2 5.5-2V4L15 6 9 4Z" }),
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("path", { d: "M9 4v14" }),
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("path", { d: "M15 6v14" })
				]
			});
		}
		function MapToggleButton(props) {
			const open = (0, react.useSyncExternalStore)(mapUi.subscribe, () => mapUi.get().open);
			const locale = (0, react.useSyncExternalStore)(subscribeLocale, activeLocale, activeLocale);
			const wide = props.wide;
			const classNames = [wide ? talk_map_module_css_default["toggleRow"] : talk_map_module_css_default["toggleButton"]];
			if (open) classNames.push(talk_map_module_css_default["toggleButtonActive"]);
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(MapIcon, {}), wide ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
					className: talk_map_module_css_default["toggleLabel"],
					children: t("map.toggle")
				}) : null]
			});
		}
		//#endregion
		...
		function apply(ctx) {
			...
			try {
				...
				ctx.effect(() => {
					const onKeyDown = (event) => {
						if (hotkeyMatches(event, canvas.get().global?.hotkey ?? "alt+KeyF")) {
							event.preventDefault();
							event.stopPropagation();
							mapUi.toggle();
						}
